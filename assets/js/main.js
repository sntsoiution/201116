...
var h;
var section = document.querySelectorAll('main section, footer');
$(window).resize(function () {
    h = $(window).height();
    //현재의 브라우저 높이를 구한다.
    $("main section").height(h).css({ paddingTop: '100px', boxSizing: 'border-box' });
    //구한 높이값(h) 가져와서 컨텐츠영역(section)의 높이에 대입
    for (var _i = 0, section_1 = section; _i < section_1.length; _i++) {
        var item = section_1[_i];
        if (h < 700) {
            item.removeEventListener('wheel', wheeling);
        }
        else {
            item.addEventListener('wheel', wheeling);
        }
    }
});
